...
Training_Data_One = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Lin_PPGFourier_300.txt', header = None, delim_whitespace = True)
Training_Data_Two = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Wu_PPGFourier_300.txt', header = None, delim_whitespace = True)
Training_Data_Three = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Liu_PPGFourier_300.txt', header = None, delim_whitespace = True)
Training_Data_Four = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Kuo_PPGFourier_300.txt', header = None, delim_whitespace = True)
Training_Data_Five = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Wang_PPGFourier_300.txt', header = None, delim_whitespace = True)
Training_Data_Six = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Li_PPGFourier_300.txt', header = None, delim_whitespace = True)
Training_Data_Seven = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Su_PPGFourier_300.txt', header = None, delim_whitespace = True)
...
y = y[randomize]
init_y=y;
train_y=init_y[:training_data_num]
test_y=init_y[training_data_num:]
...
#PCA------------------------------
pca = LDA(n_components=70)
x_train_pca = pca.fit_transform(x_train, train_y)
x_test_pca = pca.transform(x_test)
print('训练集数据的原始维度是：{}'.format(x_train.shape))
print('PCA降维后训练集数据是：{}'.format(x_train_pca.shape))
x_train=x_train_pca
x_test=x_test_pca


#Write LDA
Write_LDA = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Kuo_PPGFourier_300.txt', header = None, delim_whitespace = True)
Write_LDA=np.array(Write_LDA)
Four_Feature = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\20200225TrainData\Kuo_Norm.txt', header = None, delim_whitespace = True)
Four_Feature=np.array(Four_Feature)
total_len = min(len(Write_LDA), len(Four_Feature)/2)

pca_write = pca.transform(Write_LDA)
fp = open("Kuo_Feature_LDA_300.txt", "w")
for num in range(total_len):
    for tmp in range(len(pca_write[num])):
        fp.write(str(pca_write[num][tmp]) + ' ')
    for tmp in range(len(Four_Feature[num])):
        fp.write(str(Four_Feature[num][tmp]) + ' ')
    fp.write('\n')
...
model = Sequential()
        #將模型疊起
model.add(Dense(input_dim=6 ,units=neuron_num,activation='relu'))
...
init_data_frame=pd.crosstab(test_y, prediction, rownames=['label'], colnames=['predict'])
data_frame=pd.crosstab(test_y, prediction, rownames=['label'], colnames=['predict'],normalize=1)
