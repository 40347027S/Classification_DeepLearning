...
for num in range(total_num):
    ...
    print(file_string+'\n')
    Training_Data = pd.read_csv(r'C:\Users\Robinson\Documents\MATLAB\Data\Synthetic'+'\/' +file_string, header = None, delim_whitespace = True)
...
model = Sequential()
        #將模型疊起
model.add(Conv2D(filters=16,kernel_size=(filter_size,1),padding='same',input_shape=(raw_ppg_len,raw_ppg_width,1),activation='relu'))
model.add(MaxPooling2D(pool_size=(max_pool_size,1)))
model.add(Conv2D(filters=36,kernel_size=(filter_size ,1),padding='same',input_shape=(raw_ppg_len,raw_ppg_width,1),activation='relu'))
model.add(MaxPooling2D(pool_size=(max_pool_size,1)))
...
history = model.fit(x_train, y_train,validation_split=0.2, batch_size = 1, epochs = epoch_num, verbose=2)
